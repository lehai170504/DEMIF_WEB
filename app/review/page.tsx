...
export default function ReviewPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 font-mono">
      ...
      <main className="flex-1 container mx-auto px-6 py-8"> {/* <<< ĐÃ THÊM flex-1 */}
          ...
          <div className="col-span-1 lg:col-span-2 space-y-8">
            ...
            <div className="pt-4">
              ...
              <div className="flex gap-2 mb-6">
                <Button variant="default" size="sm" className="bg-[#FF7A00] hover:bg-[#FF8A1C] text-white">Tất Cả</Button>
                <Button variant="outline" size="sm" className="border-orange-300 text-orange-600 hover:bg-orange-50">Cần Ôn Hôm Nay</Button>
                <Button variant="outline" size="sm">Đã Thành Thạo</Button>
                <Button variant="outline" size="sm">Đang Học</Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {vocabularyItems.map((item) => (
                <Card key={item.id} className="p-6 hover:border-orange-400 transition-colors border-gray-100 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-2xl font-extrabold text-slate-800">{item.word}</h3>
                        <Badge variant="outline" className={`text-xs font-semibold px-3 py-1 ${item.difficulty === "easy" ? "bg-green-100 text-green-700 border-green-300" : item.difficulty === "medium" ? "bg-yellow-100 text-yellow-700 border-yellow-300" : "bg-red-100 text-red-700 border-red-300"}`}>
                          {getDifficultyText(item.difficulty)}
                        </Badge>
                        {new Date(item.nextReview) <= new Date() && (
                          <Badge variant="outline" className="bg-orange-100 text-[#FF7A00] border-orange-300 font-semibold">Cần Ôn</Badge>
                        )}
                      </div>
