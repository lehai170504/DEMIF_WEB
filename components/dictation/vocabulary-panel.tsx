...
export function VocabularyPanel({ lessonVocab }: { lessonVocab: string[] }) {
  ...
  return (
    <Card className="border border-white/10 bg-[#18181b] rounded-[2rem] shadow-xl overflow-hidden flex flex-col max-h-[calc(100vh-120px)] sticky top-24">
      <CardContent className="p-0 flex flex-col h-full">
        {/* HEADER */}
        <div className="p-6 pb-4 flex items-center justify-between border-b border-white/5 bg-white/5 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-white">Sổ tay từ vựng</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 scrollbar-none min-h-[200px]">
          <AnimatePresence initial={false}>
            {vocabList.length === 0 ? (
              ...
              >
                <PlusCircle className="w-10 h-10 text-zinc-700 mb-3" />
              ...
              vocabList.map((item, index) => (
                <motion.div
                  ...
                  className="group relative p-3 rounded-xl bg-white/5 border border-transparent hover:border-white/10 hover:bg-white/10 transition-all duration-200"
                >
